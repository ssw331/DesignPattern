...
/**
 * 实现 Runnable 接口的观光小火车任务类。
 * 用于模拟从对象池中借用观光小火车，执行任务，然后将其返回的过程。
 */
public class StartTouring implements Runnable {
    ...
    /**
     * 任务执行的方法。
     * 从对象池中借用一辆观光小火车，模拟游览活动，然后将车辆返回到对象池。
     */
    @Override
    public void run() {
        // 从池中借用观光小火车
        ...
        // 模拟观光小火车出发的情景
        // 线程threadId从对象池中借用了编号为bus.getBusId()的观光小火车
        System.out.printf("游览线路 %d 已启动：编号 %d 的观光小火车上游客已上车，正前往观光地点。\n", threadId, bus.getBusId());
        ...
        System.out.printf("游览线路 %d 已结束：编号 %d 的观光小火车已返回，游客已下车，车辆准备迎接下一批游客。\n", threadId, bus.getBusId());
    }
}

...
/**
 * 观光小火车抽象类，定义了观光小火车的基本属性和行为。
 * 实现 Cloneable 和 Bus 接口，允许观光小火车被克隆和运行。
...
public abstract class TourBus implements Cloneable, Bus {
    ...
    // 观光小火车的颜色，使用桥接模式中的 PaintColor 接口
    private PaintColor color;

    /**
     * 带颜色参数的构造函数。
     * 使用桥接模式中的 PaintColor 接口设置观光小火车的颜色。
     *
     * @param color 观光小火车的颜色
     */
    ...
    /**
     * 默认构造函数。自动生成观光小火车的唯一编号，并打印就绪信息。
     * 观光小火车在创建时会自动分配一个唯一编号。
     */
    public TourBus() {
        this.busId = TourBus.curBusNum.incrementAndGet();
        System.out.println("观光小火车编号 " + busId + " 已准备就绪。");
    }

    /**
     * 克隆方法，用于创建当前对象的副本。
     * 允许通过克隆现有的观光小火车实例来创建新的实例。
     *
     * @return 克隆出的新对象
     */
    @Override
    ...
    @Override
    public void run() {
        System.out.println("观光小火车编号 " + busId + " 开始出发。");
    }

    /**
     * 打印观光小火车的详细描述，包括颜色、座位数量和编号。
     * 描述包含观光小火车的颜色、座位数和唯一编号。
     */
    @Override
    public void desc() {
